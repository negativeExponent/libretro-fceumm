...
static void palette_switch_init(void) {
	...
	for (opt_def = opt_defs; opt_def->key; opt_def++) {
		if (!strcmp(opt_def->key, "fceumm_next_palette")) {
			break;
		}
	}
	...
	for (i = 0; i < PALETTE_TOTAL_COUNT; i++) {
		...
		if (opt_defs_intl) {
			...
			for (opt_def_intl = opt_defs_intl; opt_def_intl->key; opt_def_intl++) {
				if (!strcmp(opt_def_intl->key, "fceumm_next_palette")) {
					size_t j = 0;
...
static bool update_option_visibility(void) {
	...
	var.key   = "fceumm_next_show_adv_system_options";
	var.value = NULL;
	...
	var.key   = "fceumm_next_show_adv_sound_options";
	var.value = NULL;
...
static void set_variables(void) {
	...
	if (libretro_supports_option_categories) {
		option_display.key = "fceumm_next_show_adv_system_options";
		environ_cb(RETRO_ENVIRONMENT_SET_CORE_OPTIONS_DISPLAY, &option_display);

		option_display.key = "fceumm_next_show_adv_sound_options";
		environ_cb(RETRO_ENVIRONMENT_SET_CORE_OPTIONS_DISPLAY, &option_display);
	}
	...
	if (GameInfo && (GameInfo->type == GIT_VSUNI)) {
		option_display.key = "fceumm_next_palette";
		environ_cb(RETRO_ENVIRONMENT_SET_CORE_OPTIONS_DISPLAY, &option_display);
...
static void check_game_genie_variable(void) {
	...
	var.key = "fceumm_next_game_genie";
...
static void check_variables_volume_levels(void) {
	struct {
		...
		{ SND_SQUARE1, "fceumm_next_apu_square_1" },
		{ SND_SQUARE2, "fceumm_next_apu_square_2" },
		{ SND_TRIANGLE, "fceumm_next_apu_triangle" },
		{ SND_NOISE, "fceumm_next_apu_noise" },
		{ SND_DMC, "fceumm_next_apu_dpcm" },
		{ SND_FDS, "fceumm_next_apu_fds" },
		{ SND_S5B, "fceumm_next_apu_s5b" },
		{ SND_N163, "fceumm_next_apu_n163" },
		{ SND_VRC6, "fceumm_next_apu_vrc6" },
		{ SND_VRC7, "fceumm_next_apu_vrc7" },
		{ SND_MMC5, "fceumm_next_apu_mmc5" },
	};
	struct retro_variable var = { 0 };
	int i = 0;
...
static void check_variables(bool startup) {
	...
	var.key = "fceumm_next_sound_rate";
...
#if defined(HAVE_NTSC_FILTER)
	var.key = "fceumm_next_ntsc_filter";
	...
	var.key = "fceumm_next_ppu_disable_emphasis";
	if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value) {
		FSettings.DisableEmphasis = strcmp(var.value, "enabled") ? TRUE : FALSE;
	}

	FCEUI_GetRenderPlanes(&nes_sprites, &nes_background);

	var.key = "fceumm_next_hide_sprites";
	if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value) {
		nes_sprites = strcmp(var.value, "enabled");
	}

	var.key = "fceumm_next_hide_background";
	if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value) {
	...
	var.key = "fceumm_next_up_down_allowed";
	...
	var.key = "fceumm_next_nospritelimit";
	...
	var.key = "fceumm_next_overclocking";
	...
	var.key = "fceumm_next_zapper_tolerance";

	if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value) {
		FCEU_ZapperSetTolerance(atoi(var.value));
	}

	var.key = "fceumm_next_zapper_trigger";
	...
	var.key = "fceumm_next_zapper_sensor";
	...
	var.key = "fceumm_next_arkanoid_mode";
	...
	var.key = "fceumm_next_mouse_sensitivity";
	...
	var.key = "fceumm_next_show_crosshair";
...
#else
	var.key = "fceumm_next_overscan_left";
	...
	var.key = "fceumm_next_overscan_right";
	...
	var.key = "fceumm_next_overscan_top";
	...
	var.key = "fceumm_next_overscan_bottom";
	...
	var.key = "fceumm_next_use_newppu";
	...
	var.key = "fceumm_next_turbo_enable";
	...
	var.key = "fceumm_next_sndlowpass";
	...
	var.key = "fceumm_next_opll_tone";

	if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value) {
		int val = FSettings.VRC7ToneType;
		if (!(strcmp(var.value, "Mapper default")))
			FSettings.VRC7ToneType = TONE_AUTO;
		else if (!(strcmp(var.value, "2413")))
			FSettings.VRC7ToneType = TONE_2413;
		else if (!(strcmp(var.value, "VRC7")))
			FSettings.VRC7ToneType = TONE_VRC7;
		else if (!(strcmp(var.value, "281B")))
			FSettings.VRC7ToneType = TONE_281B;
		if (FSettings.VRC7ToneType != val)
			SetSoundVariables();
	}

	var.key = "fceumm_next_swapduty";
...
#ifdef GIT_VERSION
	info->library_version = "v0.99.0" GIT_VERSION;
#else
	info->library_version = "v0.99.0";
#endif
	info->library_name  = "FCEUmm Next";
