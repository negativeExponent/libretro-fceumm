...
#define DEVICE_SIFC_FKB       RETRO_DEVICE_KEYBOARD
...
static const struct retro_controller_description pads_fc[] = {
	...
	{ "Family Basic Keyboard", DEVICE_SIFC_FKB },
	{ NULL, 0 },
};
...
static void set_input(unsigned port) {
	...
	if (port <= 1) {
		...
		case DEVICE_SIFC_EXCITINGBOXING:
			...
			break;
		case DEVICE_SIFC_FKB:
			type      = SIFC_FKB;
			inputDPtr = nes_input.FKBKeys;
...
#define MK(key) RETROK_##key

static unsigned fkbmap[0x48] = {
	MK (F1), MK (F2), MK (F3), MK (F4), MK (F5), MK (F6), MK (F7), MK (F8),
	MK (1), MK (2), MK (3), MK (4), MK (5), MK (6), MK (7), MK (8), MK (9), MK (0),
	MK (MINUS), MK (EQUALS), MK (BACKSLASH), MK (BACKSPACE), MK (ESCAPE),
	MK (q), MK (w), MK (e), MK (r), MK (t), MK (y), MK (u), MK (i), MK (o), MK (p),
	MK (TILDE), MK (LEFTBRACKET), MK (RETURN), MK (LCTRL),
	MK (a), MK (s), MK (d), MK (f), MK (g), MK (h), MK (j), MK (k), MK (l),
	MK (SEMICOLON), MK (QUOTE), MK (RIGHTBRACKET), MK (INSERT), MK (LSHIFT),
	MK (z), MK (x), MK (c), MK (v), MK (b), MK (n), MK (m),
	MK (COMMA), MK (PERIOD), MK (SLASH), MK (RALT), MK (RSHIFT), MK (LALT),
	MK (SPACE), MK (DELETE), MK (END), MK (PAGEDOWN),
	MK (UP), MK (RIGHT), MK (LEFT), MK (DOWN)
};

static void update_FKB(void) {
	int x;

	for (x = 0; x < 0x48; x++) {
		nes_input.FKBKeys[x] = input_cb(0, RETRO_DEVICE_KEYBOARD, 0, fkbmap[x]) ? 1 : 0;
	}
}
...
static unsigned FTrainerButtons[12] = {
...
static void update_FTrainer(void) {
	...
	for (x = 0; x < 12; x++) {
		...
		r |= input_cb(0, RETRO_DEVICE_KEYBOARD, 0, FTrainerButtons[x]) ? (1 << x) : 0;
	}
	...
	if (device == DEVICE_AUTO) {
		if (port <= 1) {
			...
			case SIFC_EXCITINGBOXING:
				...
				break;
			case SIFC_FKB:
				nes_input.type[4] = DEVICE_SIFC_FKB;
				break;
			default:
