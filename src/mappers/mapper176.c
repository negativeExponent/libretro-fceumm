...
static void SyncCHR(void) {
	uint32 outer = fk23_regs[2] |
		(subType == 3 ? (fk23_regs[6] << 8) : 0); /* Outer 8 KiB CHR bank. Subtype 3 has an MSB register providing more bits. */
	if (CHR_8K_MODE) {
		...
		outer = (outer << 3) & ~mask; /* From 8 KiB to 1 KiB banks. Address bits are never OR'd; they either come from
...
static void SyncPRG(void) {
	const static int mask_lut[8] ={ 0x3F, 0x1F, 0x0F, 0x00, 0x00, 0x00, 0x7F, 0xFF};
	uint32 mask = mask_lut[PRG_MODE];	/* For PRG modes 0-2, the mode# decides how many bits of the inner 8 KiB bank are used. This
	...
	switch (subType) {
		...
		case 4: /* GameStar Smart Genius Deluxe */
			...
			break;
		case 5: /* HST-162 */
			prg_base = (prg_base & 0x1F) | (reg4800 << 5);
			break;
	}
	...
	switch (PRG_MODE) {
		...
		/* 7: MMC3 with   2  MB addressable. Used byc at least on 2 games:
			...
			cbase = (INVERT_PRG ? 0x4000 : 0);
			prg_base = (prg_base << 1) & ~mask; /* from 16 to 8 KiB. Address bits are never OR'd; they either come from
												   the outer bank or from the MMC3.  */
			if (MMC3_EXTENDED) {
...
static DECLFW(Write5000) {
	if (after_power && A > 0x5010 && A != 0x5FF3) { /* Ignore writes from $5000-$500F, in particular to $5008, but not $5FF3 */
		after_power = 0;
		dipsw_enable = A >= 0x5020; /* The DIP switch change on soft-reset is enabled if the first write after power-on
									   is not to $501x */
	}
...
static DECLFW(Write8000) {
	...
	latch = V;
	if (CHR_8K_MODE) {
		SyncCHR(); /* CNROM latch updated */
	}
	if (PRG_MODE == 5) {
		SyncPRG(); /* UNROM latch has been updated */
	}

	switch (A & 0xE001) {
		case 0x8000:
			if (A & 2) {
				return; /* Confirmed on real hardware: writes to 8002 and 8003, or 9FFE and 9FFF, are ignored. Needed
						   for Dr. Mario on some of the "bouncing ball" multis. */
			}
			old_ctrl = mmc3_ctrl;
...
void BMCFK23C_Init(CartInfo *info) {
	...
	dipsw_enable = 0;
	after_power = 1;
	subType = (ROM.prg.size >= (4096 * 1024)) ? 2 : (ROM.prg.size == (64 * 1024) && ROM.chr.size == (128 * 1024)) ? 1 : 0;
	if (subType == 2) {
		CHRRAMSIZE = 256 * 1024;
	}
...
void BMCFK23CA_Init(CartInfo *info) {
	...
	dipsw_enable = 0;
	after_power = 1;
	subType = (ROM.prg.size >= (2048 * 1024)) ? 2 : 1;
	if (subType == 2) {
		CHRRAMSIZE = 256 * 1024;
	}
