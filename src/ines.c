...
static void iNES_ExecPower(void) {
	...
	if (iNESCart.trainer) {
		int x;
		for (x = 0; x < 512; x++) {
			X6502_DMW(0x7000 + x, ROM.misc.data[x]);
			if (X6502_DMR(0x7000 + x) != ROM.misc.data[x]) {
...
INES_BOARD_BEGIN()
	...
	INES_BOARD( "FFE Magic Card 1M/2M",       6, Mapper006_Init         )
	INES_BOARD( "ANROM",                      7, Mapper007_Init         )
	INES_BOARD( "FFE Magic Card 1M/2M",       8, Mapper006_Init         ) /* mapper 6, submapper 4 */
	INES_BOARD( "MMC2",                       9, Mapper009_Init         )
	INES_BOARD( "MMC4",                      10, Mapper010_Init         )
	INES_BOARD( "Color Dreams",              11, Mapper011_Init         )
	INES_BOARD( "REX DBZ 5/ FFE Magic Cart 4M", 12, Mapper012_Init      )
	...
	INES_BOARD( "FFE Magic Card",            17, Mapper006_Init         )
...
/*    INES_BOARD( "",                            135, Mapper135_Init ) */ /* Duplicate of 135 */
	...
	INES_BOARD( "Sachen 3015",              146, Mapper079_Init         ) /* iNES Mapper 146 denotes the 60-pin Sachen 3015 circuit board, which is functionally identical. */
	...
	INES_BOARD( "SA009",                    160, Mapper090_Init         ) /* iNES Mapper 160 appeared to describe variant behavior of iNES Mapper 090, but Mapper 90 fully encompasses the behavior Mapper 160 was intended to emulate. */
...
/*    INES_BOARD( "",                            251, Mapper251_Init ) */ /* No good dumps for this mapper, use UNIF version */
	...
	INES_BOARD( "PEC-586",                  257, Mapper257_Init         )
	...
	INES_BOARD( "COOLGIRL",                 342, Mapper342_Init          )
	...
	INES_BOARD( "YY860606C",                363, Mapper358_Init         )
	...
	INES_BOARD( "830506C",                  358, Mapper358_Init         ) /* is this a duplicate mapper? or wrong header */
	...
	INES_BOARD( "PEC-286 (Spanish)",        371, Mapper371_Init         )
	INES_BOARD( "SFC-12",                   372, Mapper372_Init         )
	INES_BOARD( "SFC-13",                   373, Mapper373_Init         )
	...
	INES_BOARD( "35-in-1 (NROM)",           379, Mapper379_Init         )
	...
	INES_BOARD( "Dongda Sonic REC-9388",    440, Mapper440_Init         )
	INES_BOARD( "841026C/850335C ",         441, Mapper441_Init         )
	INES_BOARD( "Golden Key",               442, Mapper442_Init         )
	...
	INES_BOARD( "971107-00G",              	462, Mapper462_Init         )
	...
	INES_BOARD( "INX_007T_V01",             470, Mapper470_Init         )
	...
	INES_BOARD( "Bung Super Game Doctor 2M/4M", 561, Mapper561_Init     )
	INES_BOARD( "Venus Turbo Game Doctor 4+/6+/6M", 562, Mapper562_Init )
	INES_BOARD( "22-in-1",                  564, Mapper564_Init         )
INES_BOARD_END()
...
int iNESLoad(const char *name, FCEUFILE *fp) {
	...
	ROM.prg.size        = header.ROM_size * PRG_PAGE_SIZE;
	ROM.chr.size        = header.VROM_size * CHR_PAGE_SIZE;
	...
	if (iNESCart.iNES2) {
		...
		if (value < 0x0F) {
			ROM.prg.size        = ((header.ROM_size) | ((header.upper_PRG_CHR_size & 0x0F) << 8)) * PRG_PAGE_SIZE;
		} else {
			ROM.prg.size        = pow(2, header.ROM_size >> 2) * ((header.ROM_size & 0x03) * 2 + 1);
		}

		value = (header.upper_PRG_CHR_size >> 4) & 0x0F;
		if (value < 0x0F) {
			ROM.chr.size        = ((header.VROM_size) | ((header.upper_PRG_CHR_size & 0xF0) << 4)) * CHR_PAGE_SIZE;
		} else {
			ROM.chr.size        = pow(2, header.VROM_size >> 2) * ((header.VROM_size & 0x03) * 2 + 1);
		}
		...
		if (!ROM.prg.size) {
			ROM.prg.size = 256 * PRG_PAGE_SIZE;
		}
	}
	...
	romSize = ROM.prg.size + ROM.chr.size;
	if (romSize > filesize) {
		...
		if (!iNESCart.MiscRoms) {
			/* in some rare cases where CHR page count is malformed and no misc roms , just recompute CHR data from remaining length */
			size_t newCHRSize = filesize - ROM.prg.size;
			FCEU_PrintError(" File contains %llu bytes of unused data.\n", filesize - romSize);
			if ((ROM.chr.size != 0) && (ROM.chr.size > newCHRSize)) {
				FCEU_PrintError(" CHRROM size is is adjusted for this unused data.\n");
				ROM.chr.size = newCHRSize;
			}
		}
	}
	...
	memset(ROM.prg.data, 0xFF, uppow2(ROM.prg.size));
	...
	if (ROM.chr.size) {
		...
		memset(ROM.chr.data, 0xFF, uppow2(ROM.chr.size));
		FCEU_fread(ROM.chr.data, 1, ROM.chr.size, fp);
	}
	...
	iNESCart.PRGCRC32 = CalcCRC32(0, ROM.prg.data, ROM.prg.size);
	iNESCart.CHRCRC32 = CalcCRC32(0, ROM.chr.data, ROM.chr.size);
	iNESCart.CRC32    = CalcCRC32(iNESCart.PRGCRC32, ROM.chr.data, ROM.chr.size);

	md5_starts(&md5);
	md5_update(&md5, ROM.prg.data, ROM.prg.size);
	if (ROM.chr.size) {
		md5_update(&md5, ROM.chr.data, ROM.chr.size);
	}
	...
	if (iNESCart.iNES2) {
		...
		if (!iNESCart.trainer && ROM.misc.size) {
			if (ROM.misc.size / 1024) {
				FCEU_printf(" Misc ROM:      %-6d KiB\n", ROM.misc.size / 1024);
			} else {
				FCEU_printf(" Misc ROM:      %-6d bytes\n", ROM.misc.size);
			}
		}
	}
	 ...
	 */
	if (ROM.chr.size) {
		SetupCartCHRMapping(0, ROM.chr.data, uppow2(ROM.chr.size), 0);
	}
		...
		if (num == tmp->number) {
			if (!ROM.chr.size) {
				if (iNESCart.iNES2) {
					...
					if (CHRRAMSIZE == 0) {
						CHRRAMSIZE = iNESCart.CHRRamSize = 8 * 1024;
						FCEU_PrintError(" NES 2.0 header but no CHR-RAM specified\n");
						FCEU_PrintError(" Setting default CHR-RAM size to 8K\n");
					}
				} else {
